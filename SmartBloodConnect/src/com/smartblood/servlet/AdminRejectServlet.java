...
public class AdminRejectServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (requestId == null) {
            FlashUtils.error(request, "A valid request ID is required for rejection.");
            response.sendRedirect(request.getContextPath() + "/admin");
            return;
        }

        boolean updated = new RequestDAO().updateRequestStatus(requestId, "Rejected");
        if (updated) {
            response.sendRedirect(request.getContextPath() + "/admin?rejected=true");
            return;
        }

        FlashUtils.error(request, "The selected request could not be rejected.");
        response.sendRedirect(request.getContextPath() + "/admin");
    }
}
